...
interface SessionEditFormProps {
  players: PokerPlayer[]
  session?: PokerSession // If provided, we're editing; if not, we're adding
  onSaveSession: (session: PokerSession) => void
  onCancel: () => void
}
...
export function SessionEditForm({
  ...
  useEffect(() => {
    if (isEditing && session) {
      ...
      try {
        const parsedDate = parse(session.date, "MMMM do, yyyy", new Date())
        setDate(parsedDate)
      } catch (error) {
        console.warn("Could not parse session date:", session.date)
        setDate(new Date())
      }

      setSessionType(session.type)
      setCommonExpenses(session.commonExpenses || 0)

      // Initialize player data based on existing session
      setPlayerData(
        players.map((player) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const sessionData: PokerSession = {
        id: isEditing && session ? session.id : nanoid(),
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle>
              {isEditing ? "Edit Session" : "Add New Poker Session"}
            </CardTitle>
